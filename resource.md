...
| How to test offline mode | Offline complaint submission and automatic sync are not implemented in this MVP. The map, authentication, uploads, and Gemini assessment require an active internet connection. A future offline deployment would require a locally hosted map dataset, on-device or locally hosted AI model, local data storage, and secure background sync when connectivity returns. |
